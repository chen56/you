...
class Cmd_gen_routes_g_dart extends Command {
  ...
  Future<void> run() async {
    ...
    var routes = PageNode.fromSync(cli.pagesDir);
    Iterable<PageNode> routeList = routes.toList().where((e) => e.pageDart.existsSync());

    var nameMaxLen = routeList.map((e) => e.flatName.length).reduce((value, element) => value > element ? value : element);
    var fields = routeList.map((page) {
    ...
    Library importCode = Library((b) => b
      ...
      ..directives.addAll(
        routeList.map((lib) {
          return code.Directive.importDeferredAs(lib.packageImportUri(cli.pubspec.name, cli.libDir), "${lib.flatName}_");
        }),
      ));
