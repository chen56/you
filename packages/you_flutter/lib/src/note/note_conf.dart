...
class SpaceConf {
  ...
  Map<String, dynamic> encode({bool includeComment = true}) {
    return {
      if (includeComment) "/* notes.g.json */": comment,
...
class NoteGJson {
  ...
  NoteGJson.decode(String jsonStr) : this.decodeJson(jsonDecode(jsonStr));

  NoteGJson.decodeJson(Map<String, dynamic> json)
      : this(
  ...
  Map<String, dynamic> encode({bool includeComment = true}) {
    return {
      if (includeComment) "/* note.g.json */": comment,
      "noteId": noteId,
    };
  }

  Future<NoteGJson> save(File file) async {
    await file.writeAsString(_encoder.convert(encode()));
    return this;
  }
