...
import 'package:note/src/params/mate_core.dart';
...
class IconDataEditor extends BaseValueEditor {
  ...
  code.Expression toCode() {
    ...
    return code.refer("IconData", "package:flutter/widgets.dart").newInstance([code.refer("0x${param.value.codePoint.toRadixString(16)}")]);
  }
...
class ObjectParamEditor extends Editor {
  ...
  code.Expression toCode() {
    ...
    var positionalArguments = filtered.where((e) => !e.value.isNamed).map((e) => e.value.toCodeExpression(editors: editors));
    var namedArguments = Map.fromEntries(filtered.where((e) => e.value.isNamed).map((e) => MapEntry(e.key, e.value.toCodeExpression(editors: editors))));
...
class UnknowTypeParamEditor extends BaseValueEditor {
  ...
  code.Expression toCode() {
    ...
    return code.literalString("无法生成的代码$result");
  }
