...
class NotesGenerator {
  ...
  Future<({File file, List<NoteLib> notes})> _gen_notes_g_dart(List<NoteLib> noteLibs) async {
    ...
    var fields = noteLibs.map((noteLib) {
      ...
      // final NoteRoute dev_devtool = put("/dev/devtool",dev_devtool_g.noteInfo(), (context,print) async => await dev_devtool_.loadLibrary().then((value) => dev_devtool_.build(context,print)));
      return """  final $noteVarNameWithPadding = put("${noteLib.noteKey}",${noteLib.asVariableName}_g.noteInfo(), (context,printc) async => await ${noteLib.asVariableName}_.loadLibrary().then((value) => ${noteLib.asVariableName}_.build(context,printc)));  """;
    }).join("\n");
